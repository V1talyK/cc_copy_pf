#res = get("http://julialang.org")
#/home/lik/julia0.6/julia/julia
...
function ma_save(data,cok)
  data=json(Dict("diff"=>data));
  res = mongoPost("save", data, cok)
  return res["data"]
end
...
@everywhere function pset_surface(r2c,r2do,r2st,date,grid,srf,cok,prx)
  data=JSON.json(Dict("calc"=> r2c,
        "dev_object"=> r2do,
        "type"=> r2st,
        "date"=> date,
        "grid"=> grid,
        "surface"=> srf))
  res = mongoPost("surface_set", data, cok, prx)
  return res
end
...
@everywhere function pget_surface(date, SuTyRef, calc_ref, r2do,prx, cok="123")
  data=JSON.json(Dict("calc"=> calc_ref,              # Ссылка на расчёт
        "dev_object"=> r2do,  # Ссылка на объект разработки
        "type"=> SuTyRef,               # Ссылка на тип поверхности
        "date"=> date))                   # Дата
  res = mongoPost("surface_get", data, cok, prx)
  return res["data"]["obj"]["surface"]
end
